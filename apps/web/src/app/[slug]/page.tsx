...
export default async function Page({ params }: Props) {
  ...
  const getBody = getBlockItemBody('block.rich-text', article.blocks as BlockProps[], (blocks) => {
    ...
    <Container>
      <div className="max-w-screen-lg mx-auto">
        <div className="flex items-center gap-3">
          <p className="text-primary-600 font-bold tracking-widest uppercase">{article.category?.name}</p>
        </div>
        <h1 className="font-bold md:text-5xl leading-normal my-6">{article.title}</h1>
        ...
        {/* start content */}
        <BlocksView blocks={article.blocks} className="mt-4 leading-loose text-lg" />
        {/* end content */}
        <ShareArticle />
